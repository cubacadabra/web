...
runtime.subscribe((snapshot) => {
  if (snapshot.account_id === hostUser?.id) hostUser = { ...hostUser, username: snapshot.profile.username };
});
await runtime.dispatch({ type: "replace_session", account_id: "a", username: "Ada" });
...
await saving;
assert.deepEqual(hostUser, { id: "a", username: "Grace", body_id: "new-avatar" });
...
requests[1].resolve({ status: 200, body: '{"user":{"id":"a","username":"Later"}}' });
await staleSave;
...
requests[2].resolve({ status: 200, body: '{"user":{"id":"b","username":"Final"}}' });
