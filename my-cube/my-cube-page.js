...
async function fetchCubeCatalog() {
  ...
  for (let page = 1; page <= 200; page += 1) {
    await runtime.dispatch({ type: "load_catalog", page, page_size: CUBE_CATALOG_PAGE_SIZE });
    const catalog = runtime.snapshot.catalog;
    if (catalog.feedback?.kind === "error") throw new Error(catalog.feedback.code);
    if (!catalog.has_next_page) {
      complete = true;
      break;
    }
  }
  if (!complete) throw new Error("catalog_pagination_limit");
  const catalog = runtime.snapshot.catalog;
